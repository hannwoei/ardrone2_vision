...
//	}
	...
	{
		new_point = 1;
		//TODO: not all detected_points are examined but only the first "flow_point_size" of detected_points are used currently
...
void findDistributedPoints(unsigned char *gray_frame, unsigned char *frame, int imW, int imH, int *count, int max_count, int MAX_COUNT, struct flowPoint flow_points[],int *flow_point_size, struct detectedPoint detected_points[], int *status)
{
	...
	int nWidth, nHeight, subimH, subimW, wInit, hInit, region, n_accu, n_run;
	nWidth = 3;
	nHeight = 3;
	subimH = (imH-imH%nHeight)/nHeight; // a few pixels (imW%nHeight) are ignored!!
	subimW = (imW-imW%nWidth)/nWidth;
	wInit = 0; hInit = 0; region = 0; n_accu = 0; n_run = 0;
		...
		for (int j=0; j<nWidth; j++)
		{
			if(!status[region] || n_accu)
			{
				...
				if(*count)
				{
					...
					{
						flow_points[region-k].x = pnts_fast[k].x + wInit;
						flow_points[region-k].y = pnts_fast[k].y + hInit;
						flow_points[region-k].prev_x = pnts_fast[k].x + wInit;
						flow_points[region-k].prev_y = pnts_fast[k].y + hInit;
						flow_points[region-k].dx = 0;
						flow_points[region-k].dy = 0;
						flow_points[region-k].new_dx = 0;
						flow_points[region-k].new_dy = 0;
					}
					n_accu -= n_run;
	...
	*flow_point_size = (region + 1) - (n_accu);
	free(subframe);
	setPointsToFlowPoints(flow_points, detected_points, flow_point_size, count, MAX_COUNT);
}
